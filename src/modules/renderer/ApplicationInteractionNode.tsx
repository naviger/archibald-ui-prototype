import { ArchimateLayer } from "../enums/enumArchimateLayer";
import { BaseNode } from "./BaseNode";
import { Position } from "../structure/Position";

export class ApplicationInteractionNode extends BaseNode {
  layer:ArchimateLayer = ArchimateLayer.Application
  
  renderIcon(pos:Position):JSX.Element {
    let p1:string = "M" + (pos.x +155) + " " + (pos.y + 25)
    p1+= "A 10 10 90 0 1 " + (pos.x + 155) + " " + (pos.y + 5)
    p1+= "v 20"

    let p2:string = "M" + (pos.x +158) + " " + (pos.y + 25)
    p2+= "A 10 10 -90 0 0 " + (pos.x + 158) + " " + (pos.y + 5)
    p2+= "v 20"
    
    return (
      <g className="card-icon">
        <path fill="white" stroke="grey" d={p1} />
        <path fill="white" stroke="grey" d={p2} />
      </g>
    )
  }
}

